...
// Handle Add/Edit
$edit_mode = false;
$book = ['id' => 0, 'title' => '', 'author' => '', 'isbn' => '', 'description' => '', 'price' => '', 'discount' => 0, 'rating' => '', 'stock' => '', 'image' => 'default-book.png', 'category_id' => ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($book_id > 0) {
            $stmt = $conn->prepare("UPDATE books SET title=?, author=?, isbn=?, description=?, price=?, discount=?, rating=?, stock=?, image=?, category_id=? WHERE id=?");
            $stmt->bind_param("ssssdddsisi", $title, $author, $isbn, $description, $price, $discount, $rating, $stock, $image, $category_id, $book_id);
            $stmt->execute();
            redirect('manage_books.php', 'Book updated successfully.', 'success');
        } else {
            $stmt = $conn->prepare("INSERT INTO books (title, author, isbn, description, price, discount, rating, stock, image, category_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssdddsis", $title, $author, $isbn, $description, $price, $discount, $rating, $stock, $image, $category_id);
            $stmt->execute();
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <span class="badge bg-danger">-<?php echo number_format($book['discount'], 0); ?>%</span>
                            <?php else: ?>
                        ...
                        <td>
                            <?php if ($book['rating']): ?>
                            <span class="text-warning">★ <?php echo number_format($book['rating'], 1); ?></span>
                            <?php else: ?>
                            <span class="text-muted">-</span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td>
                            <a href="?edit=<?php echo $book['id']; ?>" class="btn btn-sm btn-warning edit-book" 
                               ...
                               data-discount="<?php echo $book['discount']; ?>"
                               data-rating="<?php echo $book['rating']; ?>"
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" name="discount" id="book_discount" class="form-control" min="0" max="100" step="0.01" value="0" placeholder="e.g. 15">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label class="form-label">Rating (1-5)</label>
                            <input type="number" name="rating" id="book_rating" class="form-control" min="0" max="5" step="0.1" placeholder="e.g. 4.5">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-4 mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (urlParams.get('edit')) {
        ...
        if (btn) {
            ...
            document.getElementById('book_discount').value = btn.dataset.discount;
            document.getElementById('book_rating').value = btn.dataset.rating;
...
document.querySelectorAll('.edit-book').forEach(btn => {
    btn.addEventListener('click', function(e) {
        ...
        document.getElementById('book_discount').value = this.dataset.discount;
        document.getElementById('book_rating').value = this.dataset.rating;
